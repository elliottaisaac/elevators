...
class Elevator
{
    ...
    void InputFloor(int currentF, vector<int>FloorHistory){
        ...
        int goToFloor;
        cin >> goToFloor;
        newDestinations.insert(newDestinations.end(), goToFloor);
        int nextFloor = 1;
        while(nextFloor != 0){
            cout << "\nEnter next destination floor or enter '0' to start elevator \n" << endl;
            cin >> nextFloor;
            newDestinations.insert(newDestinations.end(), nextFloor);
        }
    ...
    void ChangeFloors(int current, vector<int> FloorHistory, vector<int> newDestinations ){
        for(int e = 0; e <= newDestinations.size(); e++){
            ...
            int s = 1;
                ...
                for(int f = FloorHistory.back(); f != newDestinations[e] && f <= topFloor && f >= 0; f = f + s){
                    if(f == topFloor && s == 1){
                        newDestinations[e] = topFloor;
                        f = topFloor - 1;
